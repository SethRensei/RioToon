...
class VoteRepository
{
    ...
    {
        $query = $this->connec->prepare("SELECT * FROM webtoon WHERE w_id = :web");
    ...
    public function like(int $webtoon, $user)
    {
        if ($this->vote($webtoon, $user, 1)) {
            ...
            $this->connec->query("UPDATE webtoon SET likes = likes + 1 $sql_part WHERE w_id = {$webtoon}");
            return true;
        } else {
            $this->connec->query("UPDATE webtoon SET likes = likes - 1 WHERE w_id = {$webtoon}");
        }
        return false;
    ...
    public function dislike(int $webtoon, $user)
    {
        if ($this->vote($webtoon, $user, -1)) {
            ...
            $this->connec->query("UPDATE webtoon SET dislikes = dislikes + 1 $sql_part WHERE w_id = {$webtoon}");
            return true;
        } else {
            $this->connec->query("UPDATE webtoon SET dislikes = dislikes - 1 WHERE w_id = {$webtoon}");
        }
        return false;
    }
    ...
    private function vote(int $webtoon, $user, int $vote)
    {
        ...
        $query = $this->connec->prepare('SELECT v_id, vote FROM vote WHERE webtoon = :web AND "user" = :user');
        $query->bindValue(':web', $webtoon);
        ...
        if ($vote_row) {
            if ($vote_row->vote == $vote) {
                $this->connec->query('DELETE FROM vote WHERE v_id = ' . $vote_row->v_id);
                return false;
            }
            $this->former_vote = $vote_row;
            $query = $this->connec->prepare("UPDATE vote SET vote = ? WHERE v_id = {$vote_row->v_id}")->execute([$vote]);
            return true;
        }
    ...
    {
        $query = $this->connec->prepare("SELECT COUNT(v_id) as count, vote FROM vote WHERE webtoon = :web GROUP BY vote");
        $query->bindValue(':web', $webtoon);
        $query->execute();
        ...
        $query = $this->connec->query("UPDATE webtoon SET likes = {$counts[1]}, dislikes = {$counts[-1]} WHERE w_id = {$webtoon}");
        return true;
    }
